...
export class CombattimentoService {
    ...
    getCombattenteById(id: string): Combattente | undefined {
        return this.combattenti.find(c => c.id === id);
    }

    removeCombattente(id: string): void {
        this.combattenti = this.combattenti.filter(c => c.id !== id);
    }

    // INIZIO COMBATTIMENTO
    getNomeRandom(): string {
        const nomiDefault = ['Rosso', 'Blu', 'Giallo', 'Verde', 'Arancione',
            'Viola', 'Nero', 'Bianco', 'Marrone', 'Grigio',
            'Celeste', 'Magenta', 'Ciano', 'Arancio', 'Indaco',
            'Avorio', 'Cacao', 'Citrino', 'Dorato', 'Ebano',
            'Kaki', 'Lavanda', 'Nocciola', 'Fucsia',
            'Oliva', 'Perla', 'Rosa', 'Salmone', 'Turchese'];
    ...
    addCombattente(nomeSquadra: string, bonusIniziativa: number, gradoSfida: string, nomePersonaggio: string, classeArmatura: number, tipoCombettente: 'mischia' | 'distanza' = 'mischia'): void {
    ...
    posizionamento(mappa: { [key: string]: string[] }, righe: number, colonne: number): void {
    ...
    vitalita_personaggio(id: string, bonus: number): void {
